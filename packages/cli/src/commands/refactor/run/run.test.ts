...
import { runRefactor } from "./run.ts";
...
describe("runRefactor", () => {
  it("writes 'not yet implemented' message to stdout", async () => {
    ...
    await runRefactor({ target: "extract service", scope: undefined, gates: "test,lint,build" }, { stdout });
  ...
  it("message includes operation identifier", async () => {
    const { stdout, lines } = makeStdout();
    await runRefactor({ target: "extract service", scope: undefined, gates: "test,lint,build" }, { stdout });
    expect(lines.join("")).toContain("refactor run");
  });
});
